...
- [ ] Realtime admin notification (WebSocket + persisted) when a new user registers
- [ ] Realtime admin notification (WebSocket + persisted) when a user activates a trial
...
|----------|-----------|---------|
| Register/trial notifications go to **admins only** | Matches existing `new_order` pattern; goal is admin visibility ("bắn về" dashboard) | — Pending |
| Reuse `createNotification` for both events | Already does DB persist + Socket.IO emit + frontend renders unknown types | — Pending |
| Dynamic import of `createNotification` inside `auth.js` register | Avoids the `auth.js ↔ notifications.js` circular import at startup | — Pending |
